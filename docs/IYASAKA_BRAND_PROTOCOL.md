...
* **専門用語を「体温」に変換：**
    ...
    * ⭕「現場の知恵を蓄積しました」
* **問い合わせの一本化：**
    * 全ての製品問い合わせは、公式サイト（`/contact?p_id=xxx`）へ誘導する。
    * 詳細は `docs/IYASAKA_LEAD_CAPTURE_PROTOCOL.md` を参照し、技術プロトコルを遵守すること。
...
> 上記の『IYASAKA ブランド・マスター・プロトコル』および『IYASAKA Lead-Capture Technical Protocol』を全て読み込み、このプロダクトがIYASAKAの全体像の中で果たすべき『整える/つなぐ/育てる』の役割を定義してください。
> その上で、UIの文言、機能の優先順位、LPの構成案、およびリード獲得の技術実装において、IYASAKAのナラティブ（不から光へ）と共通プロトコルを一貫して表現してください。」
